...
class CartNotifier extends StateNotifier<CartState> {
  ...
  Future<void> addToCart(String productId, {int quantity = 1}) async {
    try {
      ...
      state = state.copyWith(items: cart.items, totalPrice: cart.totalPrice);
    } catch (e) {
      state = state.copyWith(errorMessage: e.toString());
    }
  }

  /// `productId` is the id of the product, not the cart item's id.
  Future<void> updateQuantity(String productId, int quantity) async {
    print('Updating quantity: productId=$productId, quantity=$quantity');
    try {
      final cart = await _cartRemoteDatasource.updateCartItem(
        productId,
        quantity,
      );
      print('Update response: items=${cart.items.length}');
      state = state.copyWith(items: cart.items, totalPrice: cart.totalPrice);
    } catch (e) {
  ...
  Future<void> removeFromCart(String productId) async {
    print('Removing from cart: productId=$productId');
    try {
      final cart = await _cartRemoteDatasource.removeFromCart(productId);
      print('Remove response: items=${cart.items.length}');
      state = state.copyWith(items: cart.items, totalPrice: cart.totalPrice);
    } catch (e) {
